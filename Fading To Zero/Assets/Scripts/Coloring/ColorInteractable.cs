...
public class ColorInteractable : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            ColorManager manager = GetComponent<ColorManager>();
            if (manager != null && !manager.IsColorActive())
            {
                manager.OpenColoring();
            }
        }

        if (ColorManager.IsAnyColorActive || ColorManager.IsTipsShowing)
        {
            promptRoot.SetActive(false);
            return;
        }
    ...
    void LateUpdate()
    {
        if (IsPlayerInRange && !IsLocked && !ColorManager.IsAnyColorActive && !ColorManager.IsTipsShowing)
        {
    ...
    private void CreatePromptUI()
    {
        ...
        promptText.text = "[E] Open Coloring";
